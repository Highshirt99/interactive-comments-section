...
const Replies = ({ replies, replyingTo, username, score, time, imgsrc }) => {
    ...
    <div className=" h-fit">
      <section className="h-auto rounded-md lg:w-[500px] p-4  relative  mr-[50px] ml-[30px] pl-[20px] mb-[10px] bg-white">
        ...
        >
          <svg width="14" height="13" xmlns="http://www.w3.org/2000/svg">
            <path
              ...
              fill="hsl(238, 40%, 52%)"
              className={`${hoverActive && "fill-lightGrayishBlue"} duration-300 delay-100  transtion-all`}
            />
          </svg>
          <span className={`${hoverActive && "text-lightGrayishBlue"} transition-all font-[500] duration-300 delay-100 text-moderateBlue text-[14px]`}>
            Reply
          </span>
        </div>

        <div className="text-[18px]  rounded-[3px] text-moderateBlue absolute lg:left-0 lg:top-5 bottom-2  lg:ml-2  bg-lightGray lg:w-[20px] w-[55px] flex lg:flex-col justify-center items-centers gap-2 lg:h-[60px] h-6">
            ...
            <span className="text-[10px] font-[700] self-center">{score}</span>
        ...
        <div className=" text-[12px] lg:ml-5 mt-1 mb-12">
          <span className="text-moderateBlue">@{replyingTo}</span>, {replies}
        </div>
      </section>
      {showReplyBox && <UserReplyInput />}
    </div>
  );
};
