...
const UserComments = () => {
  const userComments = useSelector((state) => state.data.userComments);
  let commentToEdit;
  ...
  const inputText = useSelector(
    (state) => state.data.commentToBeEdited[0]?.content
  );
  ...
  const handleEdit = (commentId) => {
    ...
    const id = commentId
    dispatch(setCommentToBeEdited(id));
    // setUserInput(inputText)
    setShowEdit(true);
    commentToEdit = userComments.filter((item) => item.id === id)[0];
    // userComments.forEach((item) => {
    //   if (selectedComment?.id === item.id) {
    //     setShowEdit(true);
    //   }
    // })
  ...
  const updateComment = () => {
    setShowEdit(false);
    setEdited(selectedComment.content);

    // dispatch(setEditedComment(edited));
  };

  const newComment = useSelector((state) => state.data.editedComment);
            ...
            <div className="mb-12 lg:ml-5">
              <div>
                <p className={`${showEdit ? "hidden" : "block"} `}>
                  {comment.id === selectedComment?.id && newComment
                    ? newComment
                    : comment.content}
                </p>
              </div>
              ...
              <div className="mt-10 mb-20">
                <textarea
                  ...
                  onChange={(e) => setSelectedComment({...selectedComment,
              ...
              {showEdit && (
                <div className="mt-10 mb-20">
                  <textarea
                    id="comment"
                    name="reply"
                    value={newComment || selectedComment.content}
                    onChange={(e) =>
                      setSelectedComment({
                        ...selectedComment,
                        content: e.target.value,
                      }) & dispatch(setEditedComment(e.target.value))
                    }
                    className=" min-h-[50px]  lg:w-[400px] w-full mb-2 p-1 outline-none lg:min-h-[100px]  bg-none border border-moderateBlue  text-darkBlue rounded-md "
                  />
